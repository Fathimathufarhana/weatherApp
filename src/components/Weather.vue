...
export default {
  ...
  methods: {
    async fetchData(latitude, longitude) {
      ...
      try {
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=8a9f1637e015ea419ad6255e9a66073f`);
...
<template>
  <div class="weather-app p-4 flex">
    <main class="main text-center relative z-10 bg-transparent ml-0">
      <!-- Weather information -->
      <div class="weather-info mb-4 p-8 rounded-md">
